...
nonisolated enum OfflineActivation {
    ...
    // The HMAC seed is sourced at runtime from the bundle Info.plist key
    // `AI_OFFLINE_ACTIVATION_SEED`, which is populated from the xcconfig
    // variable of the same name. The watch verifier and the offline key
    // generator MUST read this same key so legacy + compact codes stay
    // mutually valid across both targets.
    //
    // When the Info.plist key is absent (e.g. unit-test bundles, or a dev
    // build that hasn't set the variable), we fall back to a strong,
    // deterministic compiled-in default so make/decode round-trips remain
    // self-consistent within a process. This default is shared and stable,
    // not a per-build random, on purpose: a random fallback would make
    // codes generated by the keygen undecodable by the watch.
    static let infoPlistSeedKey = "AI_OFFLINE_ACTIVATION_SEED"

    // Strong generated default. Not secret in the cryptographic sense once
    // shipped (it lives in the binary), which is exactly why production
    // builds should override it via the Info.plist key above. See the
    // migration note: the long-term fix is asymmetric signing where only a
    // public verify key is embedded on-device.
    private static let defaultSharedSecretSeed =
        "295d459ccdd8782f6528852bd948958967305170d84d00883ab6e09dc276c6dc"

    private static var sharedSecretSeed: String {
        if let seed = (Bundle.main.object(forInfoDictionaryKey: infoPlistSeedKey) as? String)?
            .trimmingCharacters(in: .whitespacesAndNewlines),
            seed.isEmpty == false {
            return seed
        }

        return defaultSharedSecretSeed
    }
