...
describe("GET /api/v1/billing/catalog", () => {
  ...
  it("returns plans + meteringPolicy unauthenticated", async () => {
    const res = await catalog(makeRequest({ url: "http://test/api/v1/billing/catalog" }));
...
describe("GET /api/v1/account/status", () => {
  ...
  it("returns 401 for unauthenticated x-aichat-device-id (no bypass)", async () => {
    await bootstrap(
      makeRequest({
  ...
  it("returns the bound account when called with a client key, but never the keyValue", async () => {
    ...
    expect(body.key).not.toHaveProperty("keyValue");
  });

  it("admin token + x-aichat-device-id resolves the account but redacts keyValue", async () => {
    await bootstrap(
      makeRequest({
        url: "http://test/api/v1/activation/bootstrap",
        method: "POST",
    ...
    const res = await status(
      makeRequest({
        ...
        headers: {
          ...
          "x-aichat-device-id": "d3",
        },
      }),
    );
    expect(res.status).toBe(200);
    const body = (await res.json()) as { key?: Record<string, unknown> };
    expect(body.key).toBeDefined();
    expect(body.key).not.toHaveProperty("keyValue");
  });

  it("admin token without x-aichat-device-id returns 400", async () => {
    const res = await status(
      makeRequest({
        url: "http://test/api/v1/account/status",
