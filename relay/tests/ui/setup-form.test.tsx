...
describe("SetupForm", () => {
  ...
  it("walks through steps 1→2→3 and shows the Bearer Token banner", async () => {
    ...
    fireEvent.click(screen.getByRole("button", { name: /下一步/ }));
    // "Bearer Token" appears twice on step 3: as the Stepper label and as the
    // alert title. Both rendering paths are intentional, so just assert the
    // alert is present at all.
    expect(screen.getAllByText("Bearer Token").length).toBeGreaterThanOrEqual(1);
