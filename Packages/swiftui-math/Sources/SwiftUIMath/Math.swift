...
public struct Math: View {
  ...
  public static func validationError(for latex: String) -> ValidationError? {
    var parserError: ParserError?
    _ = Parser.build(fromString: latex, error: &parserError)

    guard let parserError else {
      return nil
    }

    return ValidationError(
      category: .init(parserError.code),
...
extension Math.ValidationError.Category {
  fileprivate init(_ code: Math.ParserError.Code) {
    switch code {
    case .invalidCommand, .internalError:
      self = .unsupported
